...
namespace FITApp.EmployeesService.Validators
{
    public class EducationDtoValidator : AbstractValidator<EducationDto>
    {
        ...
        {
            RuleFor(dto => dto.University).NotEmpty().WithMessage("University name is required.");
            RuleFor(dto => dto.Specialization).NotEmpty().WithMessage("Specialization is required.");
            RuleFor(dto => dto.DiplomaDateOfIssue).NotEmpty().WithMessage("Diploma date of issue is required.")
